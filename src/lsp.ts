...
export class LSP {
    ...
    constructor(public connection: IConnection) {
        ...
        this.gmlDocumentation = JSON.parse(fse.readFileSync(path.join(__dirname, path.normalize("../lib/gmlDocs.json")), "utf-8"));
    ...
    public async changedTextDocument(uri: string, contentChanges: Array<TextDocumentContentChangeEvent>) {
        ...
        this.connection.sendDiagnostics(
            DiagnosticsPackage.create(uri, finalDiagnosticPackage)
        );
    }
    //#endregion
    ...
    public async runSemantics(thisDiagnostic: DiagnosticHandler, lintPackage: LintPackage, bit: SemanticsOption) {
        ...
        if ((bit & SemanticsOption.Function) == SemanticsOption.Function) {
            diagnosticArray = diagnosticArray.concat(await this.semanticLint(thisDiagnostic, lintPackage, diagnosticArray));
        }
    ...
    public async createObject(objectPackage: CreateObjPackage) {
        ...
        if (typeof objectPackage.objectEvents == "string") {
            ...
            objectPackage.objectEvents = objectPackage.objectEvents.map(function (x) { return x.trim()});
        }
        ...
        if (this.isValidResourceName(objectPackage.objectName) == false) {
            this.connection.window.showErrorMessage("Invalid object name given. Resource names should only contain 0-9, a-z, A-Z, or _, and they should not start with 0-9.");
            return null;
        }
    ...
    public async addEvents(eventsPackage: {uri: string, events: string}) {
        ...
        eventsArray = eventsArray.map(function (x) { return x.trim()});
