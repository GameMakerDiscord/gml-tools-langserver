    ...
    public async onSignatureRequest(params: TextDocumentPositionParams): Promise<SignatureHelp|null> {
            ...
            switch (state) {
                ...
                case SignatureWalkState.INTERMEDIARY_OPEN:
                    switch (element.tokenName) {
                        ...
                        case TokenNames.oParens:
                            functionDepth++;
                            state = functionDepth >= 0 ? SignatureWalkState.FINAL_OPEN : SignatureWalkState.INTERMEDIARY_OPEN;
                            break;
                    }
                    break;
            }
        ...
        if (ourFunc) {
            ...
            referencePackage.parameters.forEach(param => {
                paras.push(ParameterInformation.create(param.label, param.documentation.slice(0, param.documentation.indexOf("."))));
            });
