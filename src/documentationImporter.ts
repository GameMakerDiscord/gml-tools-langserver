import { GMLDocs, GMLToolsSettings, SpecialDocTypes } from './declarations';
...
export class DocumentationImporter {
            ...
            if (
                ...
                if (resourceType == GMLDocs.DocType.variable) {
                    ...
                    const thisVar: GMLDocs.DocVariable = {
                        ...
                        object: SpecialDocTypes.Constant,
                    ...
                    if (this.variableValidator(thisVar)) {
                        ...
                        if (fnamesParse.InstanceVar.includes(thisVar.name)) {
                            thisVar.object = SpecialDocTypes.InstVar;
                        }
    ...
    private async parsefnames(fpath: string): Promise<FnamesParse | null> {
        ...
        for (const thisLine of ourLines) {
            // Ignore comments
            if (/^\/*.*/) continue;
            ...
            // Constants
            if (thisLine.includes(SpecialDocTypes.Constant)) {
                ourParse.Constants.push(baseWord);
            }

            // Instance Variable
            if (thisLine.includes(SpecialDocTypes.InstVar)) {
                ourParse.InstanceVar.push(baseWord);
            }

            // Obsolete
            if (thisLine.includes(SpecialDocTypes.Obsolete)) {
                ourParse.Obsolete.push(baseWord);
            }

            // ReadOnly
            if (thisLine.includes(SpecialDocTypes.Readonly)) {
